 ...
 * @since JDK 1.8
 */
public class RefundOrAlterView extends BaseFrame {
	...
	public RefundOrAlterView(ActionListener actionListener) {
	...
	private void init() {
		...
		confirmBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if ("".equals(inputItem)) {
					JOptionPane.showMessageDialog(RefundOrAlterView.this, "请输入订单号或乘机人身份证号！",
					...
					if (inputItem.length()  < 13) {
						if (!orderNo || !identityNo) {
							JOptionPane.showMessageDialog(RefundOrAlterView.this, "订单编号或身份证号输入错误，请重新输入！",
									"提示信息", JOptionPane.ERROR_MESSAGE);
							return;
						}
					}
					...
					// 传递订单号或身份证号到订单窗口
					RefundOrAlterView.this.setVisible(false);
					AlterOrderView alterOrderView = new AlterOrderView(actionListener,inputItem);
